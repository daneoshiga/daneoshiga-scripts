#!/bin/bash
#
# $Id: mashpodder.sh 34 2011-05-10 19:20:09Z chess.griffin@gmail.com $
#
        ...
        for ((i = $TOTAL - 1;i>=0;i--)); do
            ...
            if ! grep -x "^$FILENAME" $PODLOG > /dev/null; then
                ...
                if [ ! -e $DATADIR/"$FILENAME" ]; then
                    ...
                    cd $INCOMING
                    wget $WGET_QUIET -c -T $WGET_TIMEOUT --limit-rate $WGET_SPEEDLIMIT -O "$FILENAME" \
                        "$DLURL"
                    ((NEWDL=NEWDL+1))
